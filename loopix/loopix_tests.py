...
@pytest.fixture
def test_smtp_message():
    from email.mime.text import MIMEText
    sender = "secretsender@example.com"
    recipients = ["client2@localhost"]
...
def test_client_put_intp_buffer(test_smtp_message):
    client = env.clients[0]
    print client.befriended_clients
    client.put_into_buffer(test_smtp_message)
    assert client.output_buffer.qsize() == 1 
