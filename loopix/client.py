...
class Client(DatagramProtocol):
    ...
    def takeProvidersData(self, database, providerId):
        """ Function takes public information about a selected provider
            ...
            from the database. 
                ...
                providerId (int) - identifier of a provider whoes information
                                    we want to pull.
        """
        def resolve_to_ip(IPAddrs, name, port, pkey):
        ...
        try:
            ...
            provider = reactor.resolve(str(pData[3])).addCallback(save_as_ip, name=str(pData[1]), port=pData[2], pkey=petlib.pack.decode(pData[4]))
            print provider
            return provider
        except Exception, e:
            print "ERROR: ", str(e)
