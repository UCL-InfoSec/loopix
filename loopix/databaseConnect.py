...
def createUsersTable(db, tableName):
	c = db.cursor()
	c.execute('''CREATE TABLE IF NOT EXISTS %s (id INTEGER PRIMARY KEY, name blob, port integer, host text, pubk blob, provider blob)'''%tableName)
	db.commit()
	print "Table [%s] created succesfully." % tableName

def createProvidersTable(db, tableName):
	c = db.cursor()
	c.execute('''CREATE TABLE IF NOT EXISTS %s (id INTEGER PRIMARY KEY, name blob, port integer, host text, pubk blob)'''%tableName)
	db.commit()
	print "Table [%s] created succesfully." % tableName

def createMixnodesTable(db, tableName):
	c = db.cursor()
	c.execute('''CREATE TABLE IF NOT EXISTS %s (id INTEGER PRIMARY KEY, name blob, port integer, host text, pubk blob, groupId integer)'''%tableName)
	db.commit()
	print "Table [%s] created succesfully." % tableName	
